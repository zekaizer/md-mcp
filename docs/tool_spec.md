...
`query`는 공백으로 쪼개져 각 키워드가 **부분 문자열 AND**로 매칭된다 — 구(phrase) 검색도 근접도도 아니므로 `"전역 지침"`은 두 조각이 노트 어디에 있든(순서 무관) 매칭된다. CJK(한글·가나·한자) 키워드는 **띄어쓰기·인라인 스타일에 무관하게** 매칭한다(§4 CJK 갭 폴딩): `전역지침`이 `전역 지침`·`**전역** 지침`을 찾는다. 반대 방향(`전역 지침` → `전역지침`)은 키워드 분해로 이미 성립한다. `mode:filename`은 `query`를 쪼개지 않고 통째로 substring 비교하므로 순서에 민감하다(CJK 갭 폴딩은 동일 적용).
...
- **대상 겹침 금지 (파괴적 batch 공통)**: 한 batch에서 두 item의 대상이 **동일하거나 포함관계**면 거부 — path 레벨(조상↔자손 디렉토리·파일: `move`의 source, `delete`의 path), 섹션 레벨(조상↔자손 heading_path: `edit_sections`). `move`는 추가로 **최종 destination 중복**·**destination이 batch 내 다른 source와 동일 또는 그 하위**(swap·chain·비워지는 경로로의 이동)도 거부 — swap은 temp 이름 경유. 예외: destination이 다른 item의 **destination** subtree 안인 중첩은 final-tree 의미론으로 허용, 조상 destination부터 적용([ADR-0024](adr/0024-unified-move-primitive.md)). 도구가 늘어도 이 원칙을 따른다.
- **heading_path 매칭·정규화**: heading_path는 **조상 heading을 포함**하므로 부모가 다른 동명 heading(`# Q1 > ## Status` vs `# Q2 > ## Status`)은 서로 다른 path라 구분된다 — `occurrence`는 *full heading_path가 완전히 같은*(같은 부모 아래 형제 중복) 경우에만 발생/필요. 그때 `occurrence` 없으면 다수 매칭 → error, 있으면 문서 순서 1-based 선택. **매칭 우선순위**: full chain **정확 일치가 우선**이며, 정확 일치가 하나도 없을 때만 suffix 매칭(짧은 경로로 깊은 섹션 지정)을 fallback으로 허용 — suffix 매칭은 유일할 때만 성립하고 `occurrence`와 조합 불가(occurrence 번호는 outline의 정확-일치 그룹 기준이므로). heading 비교 시 trailing `#`, 공백, inline markdown 정규화 기준을 명시(읽기/쓰기 도구가 동일 기준을 써야 `read_outlines`로 본 `occurrence`가 `edit_sections`에서 동일하게 해석됨). path 비교·heading_path 매칭·dedup은 **Unicode NFC 정규화** 후 수행(macOS NFD↔Linux NFC 다기기 sync 대응); `search_notes`의 본문·파일명 키워드 매칭도 동일하게 NFC 정규화 후 비교한다. heading 텍스트는 위 정규화 후 **대소문자 구분**(case-sensitive).
- **CJK 갭 폴딩 (search_notes 전용)**: 한글·가나·한자는 띄어쓰기가 필수가 아니라 `전역지침`과 `전역 지침`이 같은 말이고, 인라인 스타일(`**전역** 지침`)은 소스에서만 term을 가른다. 그래서 매칭 전에 **양옆이 모두 CJK인 갭**을 제거한 shadow 문자열로 한 번 더 스캔한다. 갭 제거 조건: 갭이 공백·`*`·`~`·`` ` ``로만 이뤄지고 **빈 줄을 포함하지 않을 것**. 나머지는 전부 경계로 남는다 — `|`(표 셀), `#`(섹션), `-`(리스트), `>`(인용), `.`(문장), 빈 줄(문단·`***` 수평선), 그리고 `_`(`전역_지침`이 식별자로 읽히는 쪽이 흔해 보수적으로 제외). **CJK 한정이 안전장치다**: `snake_case`·`a * b`는 이웃이 ASCII라 규칙이 발동하지 않는다 — markdown을 해석하는 게 아니므로 ASCII로 확장 금지. raw 스캔과 shadow 스캔은 **합집합**(폴딩이 query가 담을 수도 있는 마커를 지우므로 교체하면 raw 히트를 잃는다). snippet·match_count는 shadow가 아니라 **원본 byte offset**으로 보고한다.
...
- **상대 링크 무결성**: `move_notes`는 노트 경로를 바꿔 markdown 상대 링크 `[..](path.md)`를 깨뜨릴 수 있다. opt-in `update_links: true`가 표준 Markdown 링크를 배치와 원자적으로 재작성한다([ADR-0022](adr/0022-link-rewrite-on-move.md)); 기본값(false)에서는 종전대로 링크가 깨질 수 있다. wikilink는 여전히 범위 밖.
- **콜 간 비원자성**: 여러 도구 호출에 걸친 트랜잭션은 없다 — 예컨대 `move_notes`→`edit_properties` 2콜 중 후자가 실패해도 전자는 이미 적용된다. 각 콜이 결과를 보고하므로 에이전트가 부분완료를 감지·복구해야 한다. 무인 대규모 재구성이 필요해지면 별도 `begin/commit_transaction`을 후속 ADR로 검토.
