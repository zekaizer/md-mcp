...
def run(c: MCPClient, t: Runner, vault: str) -> None:
    ...
    r = sc(c.call("move_notes", {"moves": [{"source": "org/b.md", "dest": "org/b2.md"}]}))
    t.check("move rename-in-place (moved field)", r["ok"] and r["moved"][0]["to"] == "org/b2.md")
    t.check("move note dest no-ext SUFFIX", sc(c.call("move_notes", {"moves": [{"source": "org/b2.md", "dest": "org/b2"}]}))["errors"][0]["code"] == "SUFFIX")
    t.check("move dir dest .md SUFFIX", sc(c.call("move_notes", {"moves": [{"source": "org/sub2/", "dest": "org/sub2b.md"}]}))["errors"][0]["code"] == "SUFFIX")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/sub2/", "dest": "org/sub3"}]}))
    t.check("move dir rename (/ echo)", r["ok"] and os.path.isdir(os.path.join(vault, "org/sub3")) and r["moved"][0]["to"] == "org/sub3/")
    c.call("create_notes", {"notes": [{"path": "org/s1.md", "content": "1\n"}, {"path": "org/s2.md", "content": "2\n"}]})
    r = sc(c.call("move_notes", {"moves": [{"source": "org/s1.md", "dest": "org/s2.md"}, {"source": "org/s2.md", "dest": "org/s1.md"}], "overwrite": True}))
    t.check("move swap rejected", not r["ok"] and any(e["code"] == "BATCH_COLLISION" for e in r["errors"]))
    r = sc(c.call("move_notes", {"moves": [{"source": "org/s1.md", "dest": "org/s1.md"}]}))
    t.check("no-op move CONFLICT", not r["ok"] and r["errors"][0]["code"] == "CONFLICT")
    c.call("create_notes", {"notes": [{"path": "org/coll.md", "content": "x\n"}]})
    r = sc(c.call("move_notes", {"moves": [{"source": "org/coll.md", "dest": "org/b2.md"}]}))
    t.check("move collision CONFLICT", not r["ok"] and r["errors"][0]["code"] == "CONFLICT")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/coll.md", "dest": "org/b2.md"}], "overwrite": True}))
    t.check("move overwrite", r["ok"])
    c.call("create_notes", {"notes": [{"path": "org/e.md", "content": "e\n"}, {"path": "org/f.md", "content": "f\n"}]})
    r = sc(c.call("move_notes", {"moves": [{"source": "org/e.md", "dest": "org/newd/"}, {"source": "org/f.md", "dest": "org/newd/"}]}))
    t.check("move N->1 dir autocreate", r["ok"] and [m["to"] for m in r["moved"]] == ["org/newd/e.md", "org/newd/f.md"])
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd/e.md", "dest": "org/newd2/e2.md"}]}))
    t.check("move+rename in one call", r["ok"] and r["moved"][0]["to"] == "org/newd2/e2.md")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd2/e2.md", "dest": "org/newd/e.md"}]}))
    t.check("move+rename back", r["ok"])
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd/", "dest": "org/newd/in/"}]}))
    t.check("move own subtree rejected", not r["ok"] and r["errors"][0]["code"] == "OVERLAP")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd/e.md", "dest": "org/b2.md/"}]}))
    t.check("move dest occupied by note", not r["ok"] and r["errors"][0]["code"] == "DEST_NOT_DIR")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd/e.md", "dest": "org/"}, {"source": "org/sub3/d.md", "dest": "org/newd/"}]}))
    t.check("move independent batch ok", r["ok"])
    r = sc(c.call("move_notes", {"moves": [{"source": "org/sub3/", "dest": "org/held/"}, {"source": "org/newd/d.md", "dest": "org/held/sub3/d.md"}]}))
    t.check("move nested dest ancestor-first", r["ok"] and os.path.exists(os.path.join(vault, "org/held/sub3/d.md")))
    r = sc(c.call("move_notes", {"moves": [{"source": "org/newd/f.md", "dest": "/"}]}))
    t.check("move to vault root via '/'", r["ok"] and r["moved"][0]["to"] == "f.md")
    r = sc(c.call("move_notes", {"moves": [{"source": "org/ghost.md", "dest": "org/"}]}))
    t.check("move missing source NOT_FOUND idx", not r["ok"] and r["errors"][0]["code"] == "NOT_FOUND" and r["errors"][0].get("index") == 0)
    r = sc(c.call("move_notes", {"moves": [{"source": "한글노트.md", "dest": "한글노트2.md"}]}))
    t.check("move NFC path on NFD file", r["ok"])
